...
describe("GlpStrategy fork test", () => {
  ...
  async function setUp() {
    ...
    await (async () => {
      ...
      await sGlpDist.connect(admin).setTokensPerInterval(100_000_000_000_000n);
    })();
  ...
  it("Should set up the strategy", async () => {
    ...
    await yieldBox.withdraw(assetId, me.address, me.address, 0, shares.div(2));
    ...
    // await dump();
    await expect(strategy.harvestGmx(1, 20)).to.be.revertedWith("Not enough");
  });
});
