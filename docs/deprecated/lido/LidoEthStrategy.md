...
```solidity
function curveStEthPool() external view returns (contract ICurveEthStEthPool)
```
...
| Name | Type | Description |
|---|---|---|
| _0 | contract ICurveEthStEthPool | undefined |
...
```solidity
function stEth() external view returns (contract IStEth)
```
